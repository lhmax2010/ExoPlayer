...
| --- | --- | --- | --- | --- |
| Build / native link | Pass | `:lib-exoplayer-cppbridge:assembleDebugAndroidTest`; `:lib-exoplayer-cppbridge:testDebugUnitTest` | Codex | Native test hooks linked for all configured ABIs. |
| JNI/value smoke | Pass | `CppBridgeNativeSmokeTest`: 18/18 via `run_validation.sh --skip-build` | Codex | Includes source/media-item, timeline, tracks, metadata, opaque-token, and video-effects smoke. |
| Player/runtime smoke | Pass | `CppBridgeNativePlayerInstrumentationTest`: 100/100 via `run_validation.sh --skip-build`; full `connectedDebugAndroidTest`: 118/118 | Codex | Includes Stage 5 custom-cache-key/DRM playback smoke and local-asset player-message runtime smoke. |
| Demo manual validation | Pending | Not run in this pass | Codex | Automated cppbridge module validation passed; demo manual flow still separate. |
| Logcat review | Pending | Not run in this pass | Codex | No crash/failure surfaced from Gradle instrumentation output. |
| Release recommendation | Pass with notes | Unit + instrumentation + validation script pass on Android 16 emulator | Codex | Reduced endpoint is green; full Java `api.txt` parity gaps remain tracked as next-phase capability work. |
...
| Machine / host | Linux 6.17.0-23-generic amd64 |
| Branch / package snapshot | `codex/cppbridge-stage5-complete` at `d73f34a` before local Stage5 validation-summary commit |
| JDK | OpenJDK 17.0.18 |
| Gradle | 8.13 |
| Android SDK | `/home/linhao/Android/Sdk` |
| NDK / CMake | NDK `30.0.14904198`; CMake `4.1.2` |
| Device / emulator | `cppbridge_android16_api36`, Android 16 / API 36 |
...
| --- | --- | --- | --- |
| `CppBridgeNativeSmokeTest` | Pass | 18/18 through `run_validation.sh --skip-build --serial emulator-5554` | None for reduced endpoint. |
| `CppBridgeNativePlayerInstrumentationTest` | Pass | 100/100 through `run_validation.sh --skip-build --serial emulator-5554`; also covered in full 118/118 `connectedDebugAndroidTest` | None for reduced endpoint. |
| `run_validation.sh` aggregate result | Pass with notes | Passed using a temporary `/tmp/gradlew-bash` wrapper because this checkout's `gradlew` is not executable; direct shell execution of the script also needs executable bit. | Consider setting executable bits for `gradlew` and `scripts/cppbridge/run_validation.sh` in a future metadata-only cleanup. |
...
| --- | --- | --- | --- | --- |
| Second-batch closed | analytics aggregate / analytics-only listener | Validation-first | Pass | Android 16 can emit extra aggregate callbacks; tests now assert `beforeRemoveCb >= 2`, `afterRemoveCb == beforeRemoveCb`, and `callbackStopped=1` while still validating payloads. |
| Second-batch closed | image output reduced callback behavior | Validation-first | Pass | Android 16 bitmap allocation metadata is reflected in the smoke expectations. |
| Second-batch closed | renderer messaging reduced model | Validation-first | Pass | Timed/player-message runtime smoke now uses packaged local MP4 test assets rather than network media. |
| Second-batch closed | wake mode create-time + runtime setter | Validation-first | Pass | Runtime setter updates the bridge test snapshot and is covered by `nativeWakeModeRuntimeSmokeTest_updatesWakeMode`. |
| Second-batch closed | priority reduced wrapper/runtime state | Validation-first | Pass | Runtime registered-state expectations match current `PriorityTaskManager` behavior. |
| Second-batch closed | preload target-duration behavior | Validation-first | Pass | Create-time and runtime target-duration smoke remain covered. |
| Second-batch closed | builder-style reduced config/build path | Validation-first | Pass | Existing config/builder smoke remains covered in the 100-test player suite. |
