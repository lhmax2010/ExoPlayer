...
| current tracks | reduced `TracksSnapshot` plus grouped compatibility path | Done | `nativeCurrentTracksSmokeTest_returnsTracksSummary`; `CppBridgeNativeSmokeTest.nativeTracksSnapshotConversionSmokeTest_returnsStructuredSummary` |
...
| runtime noisy handling | create-time config only | Done | `nativePlayerConfigFlagsSmokeTest_returnsCreateTimeFlags`; `nativeBuilderConfigSmokeTest_returnsConfiguredSnapshot`; `CppBridgeNativeSmokeTest.nativeBuilderBuildSmokeTest_buildsConfiguredPlayer` |
...
| `Tracks` | Partial | reduced tracks/group/format snapshots, group token baseline, first/second group smoke visibility, listener/query coverage for representative audio/video fields | full `Tracks.Group` / `Format` parity; deeper coverage of second-group fields until it matches first-group depth; complete Java object semantics |
