...
| current tracks | reduced `TracksSnapshot` plus grouped compatibility path, including expanded `TrackInfo` bitrate/average/peak bitrate, rotation, pixel ratio, and color fields | Done | `nativeCurrentTracksSmokeTest_returnsTracksSummary`; `CppBridgeNativeSmokeTest.nativeTracksSnapshotConversionSmokeTest_returnsStructuredSummary` |
