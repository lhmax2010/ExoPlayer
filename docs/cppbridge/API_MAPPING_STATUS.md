...
| standard listener support | single standard `PlayerListener` delegate; `SetListener` replaces the previous delegate and `RemoveListener`/`SetListener(nullptr)` detaches it | Done, reduced listener model | `nativeListenerSmokeTest`; `nativeListenerDetachSmokeTest`; `nativeListenerLifecycleNegativeSmokeTest_handlesRepeatedAndNullRemoval` |
