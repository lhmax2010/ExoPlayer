...
- `factoryToken=test-injected-media-source-factory`
  - Result:
- `customCacheKey=stage5-cache-key`
  - Result:
- `drmLicenseUri=https://license.example.com/stage5`
  - Result:
- `fallbackApplied=1`
  - Result:
- `tokensIsolated=1`
...
| `nativeCurrentTracksSmokeTest_returnsTracksSummary` | `group0Id=video-group`; `group0TokenPresent=1`; `track0LabelTokenPresent=1`; `group1Id=audio-group`; `group1TokenPresent=1`; `group1Track0Id=`; `group1Track0Label=`; `group1Track0LabelTokenPresent=` | `DATA_STRUCTURE_MAPPING.md` Tracks section |  |  |
...
| `nativeCustomCacheKeyPlaybackSmokeTest_preservesCacheKeyThroughPlaybackPath` | `factoryToken=stage5-cache-fake-media-source-factory`; `injectedFactoryUsed=1`; `prepared=1`; `errorCode=0`; `mediaId=stage5-cache-item`; `sourceType=5`; `customCacheKey=stage5-cache-key` | `API_MAPPING_STATUS.md` `getCurrentMediaItem`; `DATA_STRUCTURE_MAPPING.md` `MediaItemDescriptor` |  |  |
| `nativeDrmPlaybackSmokeTest_preservesDrmConfigThroughPlaybackPath` | `factoryToken=stage5-drm-fake-media-source-factory`; `injectedFactoryUsed=1`; `prepared=1`; `errorCode=0`; `mediaId=stage5-drm-item`; `hasDrmConfiguration=1`; `drmLicenseUri=https://license.example.com/stage5`; `drmHeaderCount=2`; `drmKeySetIdLength=4` | `DATA_STRUCTURE_MAPPING.md` `MediaItem.DrmConfiguration` |  |  |
...
| `nativeAnalyticsAudioSessionIdChangedSmokeTest_reportsConcreteAnalyticsEvent` | `beforeRemoveCb>=2`; `callbackStopped=1`; `audioSessionId=42` | `API_MAPPING_STATUS.md` analytics audio session id changed; `DATA_STRUCTURE_MAPPING.md` `AudioSessionIdChangedEvent` |  |  |
